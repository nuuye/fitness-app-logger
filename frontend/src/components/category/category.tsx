...
export default function Category({
    ...
    const [subCategoryToDelete, setSubCategoryToDelete] = useState<Record<string, string> | null>(null);
            ...
            {subCategoryToDelete &&
                createPortal(
                    <WarningPopup
                        ...
                        onConfirm={() => {
                            subCategoryDeleteHandler(Object.values(subCategoryToDelete)[0]);
                            setSubCategoryToDelete(null);
                        }}
                        title="Warning"
                        text={
                            <>
                                You are about to delete all exercices related to{" "}
                                <strong>{Object.keys(subCategoryToDelete)[0]}</strong>
                            </>
                        }
                    />,
                    document.body
                            ...
                            subCategories.map((subCategory) => (
                                <div onClick={() => retrieveSubCategory(subCategory._id)} key={subCategory._id}>
                                    <SubCategory
                                        ...
                                        key={subCategory._id}
                                        onClickDelete={() =>
                                            setSubCategoryToDelete({ [subCategory.name]: subCategory._id })
                                        }
